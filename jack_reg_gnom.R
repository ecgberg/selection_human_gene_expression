...
filt_spectra <- tissue_spectra %>% mutate(gnom_sites=rowSums(.[grep('X[123456789]', names(.))], na.rm=TRUE)) %>% 
                    filter(gnom_sites>0 | is.na(X.NA.))
filt_spectra <- filt_spectra %>% replace_na(list(X1 = 0, X2=0, X3=0, X4=0, X5=0, X6=0, X7=0, X8=0, X9=0))
...
# Precompute info
gene_ids <- precompute_ids(orig_tab=filt_spectra)
...
trial_run <- run_jackknife(spec=filt_spectra, ids=gene_ids)
